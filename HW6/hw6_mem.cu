...
static uint elval_rand(int i, int n, uint *params) {
    uint r = rand();
    if(params[0] == 0) return(r);
    else return(r % params[0]);
}
...
static uint elval_seq(int i, int n, uint *params) {
    ...
    if(params[2] == 0) return(v);
    else return(v % params[3]);
}
...
Gmem_data::Gmem_data(uint n, int n_blk, int threads_per_block,
	...
	fprintf(stderr, "Gmem_data(%u, %d, %d): new failed?!\n",
		n, n_blk, threads_per_block);
	if(this->v_host == NULL) fprintf(stderr, "  v_host == NULL\n");
	if(stride_host == NULL) fprintf(stderr, "  stride_host == NULL\n");
	if(sum_host == NULL) fprintf(stderr, "  sum_host == NULL\n");
	exit(-1);
    }
...
void gmem_cpu(Gmem_data *data, int n_read) {
    ...
    for(uint my_idx = 0; my_idx < n_stride; my_idx++) { // for each "thread"
	uint my_stride = stride[my_idx];
	uint acc = 0;
	uint m = my_idx;
	for(uint j = 0; j < n_read; j++) {
...
float gmem_time(Gmem_data *data, int n_read) {
    ...
    // launch the kernel
    gmem_fetch<<<data->n_blk, data->threads_per_block>>>
              (data->v_dev, data->n, data->stride_dev, data->sum_dev, n_read);
...
void print_time(double t) {
    if(t < 1.0e-3) printf("Kernel execution time: %.4f microseconds\n", 1.0e6*t);
    else if(t < 1.0) printf("Kernel execution time: %.4f milliseconds\n", 1.0e3*t);
    else printf("Kernel execution time: %.4f seconds\n", t);
}
...
int main(int argc, char **argv) {
    ...
    if(coalesced) ai_const(&s_init, n_threads);
    else ai_rand(&s_init, n_threads);
    ai_rand(&v_init, n);
    ...
    else printf("The results%s match!\n", gmem_test(data, 4) ? "" : " don't");
