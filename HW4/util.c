...
// get_int(argc, argv, opt_names, i, v0, pos): return the i^th command line argument as an int.
...
int get_int(int argc, char **argv, const char **opt_names, int pos, int v0) {
    if(argc <= pos) return(v0);
    int neg = argv[pos][0] == '-';
    if(neg && !(argv[pos][1])) return(v0);
    // is argv[i] an integer ?
    for(int j = 0; argv[pos][j]; j++)
	if(!isdigit(argv[pos][j]))
	    usage(argc, opt_names, pos); // argv[i] is not an integer, print an error message and exit
