...
/* op.c: a relatively simple example to demonstrate the impact of branch
 * prediction. The basic idea is that we create an array of n_op "operations".
 * An operation is one of 8 predefined ways to update an accumulator.  The
 * evaluator for an array of operations is ar_op in array.c.  It's a switch
 * statement with a case for each operations.  If the operations are randomly
 * chosen, the branch prediction fails 7/8 of the time.  OTOH, if all operations
 * are the same, then branch prediction will work very well.  The difference in
 * the execution time with random operations and a single, fixed operation can
 * be used to estimate the cost (execution time) of a mispredicted branch.  The
 * point of this is to see how superscalar processors use branch prediction to
 ...
 *                   For example c5 will set every element of the ops array
 * to 5. random: each element is randomly chosen form 0..7. For any of these
 * parameters, if the command line argument is "-", then the default is used. If
 * ./op is run with fewer than 4 arguments, then the remaining parameters get
 ...
 *      good citizens when using shared machines such as
 * thetis.students.cs.ubc.ca. Please be a good student. Finally, I'll do 5 or 7
 * runs and then take the median. On thetis, I see outliers in that group of 5
 * or 7, but the median seems pretty stable if I try multiple batches of 5 or 7
 * runs.  On my laptop, the timing numbers that I see are much more stable.
 */
...
int main(int argc, char **argv) {
  const char *opt_names[] = {"merge", "n_op", "n_trials", "runs_per_trial",
                             "dist"};
