...
\begin{enumerate}\setcounter{enumi}{-1}
    \item \textbf{Collaborators} (\textbf{2 points (+1 extra credit)}).\label{q:collab}\\
          ...
          \begin{answer}
              Anthropic Claude 4.5 + VSCode Cline integration \\
              Used for: \\
              1.c. Converting timing data to a latex table \\
              2.a. Generated a helper function that creates the
              overlapping segments outputted from segment\_list (ie. LeftCol \& TopRow) \\
              2.b. Generated an explanation of my algorthim based on code + comments. \\
              2.c. Generated a boiler plate eunit test cases with different string lengths (I added on edge cases) \\
              2.d. Generated the hw5\_timing.erl for determing speedup file that (Altered for tile size comparison) \\
              Converted timing data to tex format. \\
              Organized my bullet point observations/explanations into paragraphs. \\
              3.c. Generated a shell script that runs .\/henon with different n\_blocks outputs GFLOPS.
              Tex Table generation \\
              3.e. Converted raw output to tex table \\
              3.g. Converted raw output to tex table, probed about double precision less speed-up \\
          \end{answer}
          \par
          ...
          \begin{enumerate}
                    ...
                    \begin{enumerate}
                        \item (\textbf{2 points}) If all elements of \code{op} are the same,
                              ...
                              \begin{answer}
                                  ...
                                  Since all elements of \code{op} are the same, the switch statement always goes to the same case. The predictor will mispredict on the first iteration (no history), but correctly predict all subsequent $n-8$ iterations.
                              \end{answer}
                        \item (\textbf{3 points}) If the elements of \code{op} are uniformly random with
                              ...
                              \begin{answer}
                                  ...
                                  Since \code{acc} is effectively random and \code{acc \& 7} produces values from 0 to 7, we access uniformly random elements of \code{op}. Each element randomly selects one of 8 cases. The predictor assumes the same case as the previous iteration, giving a $\frac{1}{8}$ chance of a correct prediction.
                              \end{answer}
                    \end{enumerate}
                    ...
                    \begin{enumerate}
                              ...
                              \begin{answer}
                                  \begin{center}
                                      \begin{tabular}{|l|c|c|c|}
                                          \hline
                                          \textbf{Distribution} & \textbf{n\_op} & \textbf{Mean Time (s)}  & \textbf{Std Dev (s)}    \\
                                          \hline
                                          const0                & 1000           & $1.4307 \times 10^{-6}$ & $1.1703 \times 10^{-7}$ \\
                                          \hline
                                          rand                  & 1000           & $6.7281 \times 10^{-6}$ & $2.0516 \times 10^{-6}$ \\
                                          \hline
                                      \end{tabular}
                                  \end{center}

                                  Parameters: n\_trials=100, runs\_per\_trial=10000, executed on thetis.students.cs.ubc.ca
                              \end{answer}
                        \item (\textbf{5 points}) By running \texttt{./op} with random and constant distributions for the elements
                              ...
                              \begin{answer}
                                  ...
                                  Mispredictions with const0: $\approx 1$ (first iteration only)

                                  Mispredictions with rand: $993 \times \frac{7}{8} \approx 869$

                                  Additional mispredictions with rand: $869 - 1 = 868$

                                  \textbf{Branch mispredict penalty:} $\frac{5297.4 \text{ ns}}{868} \approx \textbf{6.1 nanoseconds}$
                              \end{answer}
                        \item (\textbf{5 points}) The Intel Xeon E5-2698 v3 processors in \texttt{thetis} and \texttt{gambier}
                              ...
                              \begin{answer}
                                  base clock: $(\frac{6.1ns}{10^9 ns/s}) \cdot (2.3 \cdot 10^9 cycles / s)$ = 14.03 $\approx$ 14 cycle branch-mispredict penalty \\
                                  boost clock: $(\frac{6.1ns}{10^9 ns/s}) \cdot (3.6 \cdot 10^9 cycles / s)$ = 21.96 $\approx$ 22 cycle branch-mispredict penalty \\
                              \end{answer}
                    \end{enumerate}
              ...
              \item (\textbf{5 points})
                    ...
                    \begin{answer}
                        ...
                        const: $\frac{15888 ins}{(3.6 \cdot 10^9 cycles/s) \cdot 1.4307 \times 10^{-6}s}$ = 3.085 ins / cycle \\
                        rand: $\frac{15888 ins}{(3.6 \cdot 10^9 cycles/s) \cdot 6.7281 \times 10^{-6}s}$ = 0.656 ins / cycle
                    \end{answer}
    ...
    \item Dynamic Programming (\textbf{37 points})\label{q:dp}
          ...
          \begin{enumerate}
              ...
              \item Write a one to three paragraph describing your parallel implementation
                    ...
                    \begin{answer}
                        This implementation divides the edit distance dynamic programming tableau into rectangular tiles
                        and processes them using a chain of worker processes. The tableau is partitioned horizontally
                        into rows of tiles, where each row has a height of `TileHeight` and each tile within a row has a
                        width of `TileWidth`. The chain length is determined by the number of tile rows needed to cover
                        string S1, with a maximum of `NW` workers.
                        \\ \\
                        The key insight (as provided by the hint), is that tiles overlap at their boundaries:
                        the right column of one tile becomes the left column of its right neighbor, and the
                        bottom row of one tile becomes the top row of the tile below it. The `segment\_list/2`
                        helper function creates these overlapping segments from the input strings. Each worker
                        in the chain is responsible for computing one horizontal row of tiles. Workers use the
                        `hw5\_lib:ed\_tile/3` function to compute individual tiles, receiving their top row
                        from the previous worker in the chain and sending their bottom row to the next worker.
                        This creates a pipeline where each worker processes tiles from left to right across its
                        assigned row while coordinating with adjacent workers through message passing.
                        \\ \\
                        The master process initializes the chain with operation costs and left column segments,
                        then feeds the top row segments (representing string S2) one at a time to the first
                        worker. Each segment propagates through the chain as workers compute their respective
                        tiles. The final edit distance is extracted from the last element of the bottom
                        row produced by the final worker after processing the last tile column. This approach
                        achieves parallelism while amortizing communication costs through tile-based
                        computation, as each worker performs `TileHeight × TileWidth` operations between
                        communications.
                    \end{answer}
              \item Write some test-cases for \code{hw5:ed\_par}. (\textbf{5 points})\\
                    ...
                    \begin{answer}
                        hw5:ed\_par\_test() located at line 123.
                    \end{answer}
...
Speedup: 1.28x
                        ...
                        \item Show how your choices of configuration parameters impact the result. (\textbf{3 points})\\
                              ...
                              \begin{answer}
                                  This represents an optimistic maximum speedup of 36.83x. The implementation manually
                                  sets the number of tile rows equal to NW (number of workers), and for these
                                  5000-character strings, the tableau is perfectly square (5000×5000), which
                                  optimizes the pipeline by balancing tile dimensions. This speedup is data-dependent
                                  and reflects ideal conditions where workers remain fully utilized.
                              \end{answer}
                        \item Report the best speed-up you could get (\textbf{2 points})\\
                              ...
                              \begin{answer}
                                  36.83x speedup (Test 5: Tile Width 40, Workers 125)
                              \end{answer}
                        \item Identify causes of performance loss for this example? (\textbf{3 points})\\
                              ...
                              \begin{answer}
                                  I hypothesize that performance loss stems primarily from the chain architecture's
                                  cascading delays: if any worker stalls, all downstream workers must wait for their top row input,
                                  propagating latency through the pipeline. The algorithm is slow to start but
                                  reaches peak parallel performance (NW = number of tile rows) at around half tableau
                                  completion, then reduces active workers as they complete. Additionally, the master
                                  process passes left column segments worker-by-worker using [Head | Tail] pattern,
                                  creating sequential initialization overhead that could potentially be reduced with parallel
                                  broadcast. Note: This initialization overhead is likely negligible since
                                  each worker must wait for worker 1 to complete its first tile anyway, effectively
                                  hiding the sequential distribution cost, though parallel broadcast could be tested
                                  for confirmation. Evidence: speedup degrades when tile dimensions become imbalanced
                                  (very small or very large tile widths), suggesting communication overhead dominates
                                  when tiles don't amortize message passing costs effectively.
                              \end{answer}
                    \end{enumerate}
          \end{enumerate}
          ...
          \begin{enumerate}
              ...
              \item \textbf{(3 points)}
                    ...
                    \begin{answer}
                        48 SMs \\ \\
                        Sources: \\
                        \href{https://forums.developer.nvidia.com/t/how-much-sms-of-gtx-3070/217134}{NVIDIA developer forum} \\
                        \href{https://en.wikipedia.org/wiki/List_of_Nvidia_graphics_processing_units}{Wikipedia NVIDIA gpu spec} \\
              ...
              \item \textbf{(5 points)}
                    ...
                    \end{itemize}
                    \begin{answer}
                        The steps are appearing at multiples of 48. This correlates with number of SM's available
                        on the RTX 3070 Ti. This is also confirmed with the peak performance appearing at n\_blocks 240 (240 / 48 = 5). \\
                        \\
                        Having n\_blocks at multiples of 48 maximizes the efficeincy of block scheduling on SM's. If we have 48k + 1 blocks,
                        the extra block has to wait for an SM to finish to run independently.
                    \end{answer}
              ...
              \item \textbf{(5 points)}
                    ...
                    \begin{answer}
                        \begin{center}
                            \begin{tabular}{|l|c|c|c|}
                                \hline
                                \textbf{Configuration} & \textbf{CPU}         & \textbf{GPU}          & \textbf{Units} \\
                                \hline
                                n\_data                & 102,400              & 102,400               & N/A            \\
                                \hline
                                n\_iter                & 10,000               & 1,000,000             & iterations     \\
                                \hline
                                Mean time (5 runs)     & 3.1786               & 0.13288               & seconds        \\
                                \hline
                                Total FLOPs            & $5.12 \times 10^{9}$ & $5.12 \times 10^{11}$ & operations     \\
                                \hline
                                \textbf{GFLOPS}        & \textbf{1.611}       & \textbf{3853.5}       & GFLOPS         \\
                                \hline
                            \end{tabular}
                        \end{center}

                        \textbf{Speedup:} $\frac{3853.5}{1.611} = 2391.7\times$ \\ \\
                    \end{answer}
                    ...
                    \begin{answer}
                        \begin{center}
                            \begin{tabular}{|l|c|c|c|}
                                ...
                                n\_data                & 102,400              & 102,400              & N/A            \\
                                \hline
                                n\_iter                & 10,000               & 10,000               & iterations     \\
                                \hline
                                Mean time (5 runs)     & 3.180                & 0.09170              & seconds        \\
                                \hline
                                Total FLOPs            & $5.12 \times 10^{9}$ & $5.12 \times 10^{9}$ & operations     \\
                                \hline
                                \textbf{GFLOPS}        & \textbf{1.610}       & \textbf{55.83}       & GFLOPS         \\
                                \hline
                            \end{tabular}
                        \end{center}

                        \textbf{Speedup:} $\frac{55.83}{1.610} = 34.68\times$ \\ \\

                        Observations: Double-precision shows dramatically lower GPU speedup (34.68× vs 2391.7× for single-precision), primarily because GPUs have significantly fewer double-precision units than single-precision units.
                    \end{answer}
          \end{enumerate}
\end{enumerate}
